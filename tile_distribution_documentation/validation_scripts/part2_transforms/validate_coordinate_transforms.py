...
def test_coordinate_transforms_basics():
    """Test the coordinate transforms basics script."""
    print_step(1, "Testing coordinate transforms basics")
    
    script_path = Path(__file__).parent / "coordinate_transforms_basics.py"
    ...
    try:
        ...
        if success:
            print("✅ Coordinate transforms basics script executed successfully")
            ...
            print(f"❌ Coordinate transforms basics script failed with return code {result.returncode}")
        ...
        print("❌ Coordinate transforms basics script timed out")
        return False
    except Exception as e:
        print(f"❌ Error running coordinate transforms basics script: {e}")
        return False
...
def test_basic_transform_operations():
    """Test basic transform operations work correctly using calculate_lower_index."""
    ...
    try:
        ...
        # Test UnmergeTransform - upper (2D) to lower (1D)
        unmerge = UnmergeTransform([2, 3])
        coord_2d = MultiIndex(2, [1, 1])  # Upper: 2D coordinate
        coord_1d = unmerge.calculate_lower_index(coord_2d)
        expected_1d = [4]  # [1, 1] in 2x3 grid should be 4 in 1D (lower)
        
        if coord_1d.to_list() != expected_1d:
            print(f"❌ UnmergeTransform failed: expected {expected_1d}, got {coord_1d.to_list()}")
            return False
        print("✅ UnmergeTransform working correctly")
        
        # Test MergeTransform - upper (1D) to lower (2D)
        merge = MergeTransform([2, 3])
        coord_1d = MultiIndex(1, [4])  # Upper: 1D coordinate
        coord_2d = merge.calculate_lower_index(coord_1d)
        expected_2d = [1, 1]  # 4 in 1D should be [1, 1] in 2x3 grid (lower)
        
        if coord_2d.to_list() != expected_2d:
            print(f"❌ MergeTransform failed: expected {expected_2d}, got {coord_2d.to_list()}")
            return False
        print("✅ MergeTransform working correctly")
        
        # Test ReplicateTransform - upper (replica) to lower (empty)
        replicate = ReplicateTransform([3])
        coord_replica = MultiIndex(1, [0])  # Upper: replica index
        coord_empty = replicate.calculate_lower_index(coord_replica)
        expected_empty = []  # Should create empty lower dimension
        
        if coord_empty.to_list() != expected_empty:
            print(f"❌ ReplicateTransform failed: expected {expected_empty}, got {coord_empty.to_list()}")
            return False
...
def main():
    ...
    # Run all tests
    tests = [
        ("Coordinate transforms basics script", test_coordinate_transforms_basics),
    ...
    if all_passed:
        ...
        print("\n💡 What we validated:")
        print("  • Coordinate transforms basics script executes correctly")
        print("  • All transforms have consistent APIs")
        print("  • calculate_lower_index operations work as expected")
        print("  • Upper->Lower transformation direction is correct")
