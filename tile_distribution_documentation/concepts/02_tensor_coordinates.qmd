...
graph TB
    subgraph "MultiIndex Structure"
        ...
        D0["Dimension 0<br/>X coordinate"]
        D1["Dimension 1<br/>Y coordinate"]
        D2["Dimension 2<br/>Z coordinate"]
        DN["Dimension N-1<br/>..."]
    end

    subgraph "Usage Context"
        T["Transforms<br/>Input/Output"]
        A["Adaptors<br/>Position Tracking"]
        TV["Tensors<br/>Element Access"]
    end
...
from pytensor.tensor import MultiIndex

# Create a 3D coordinate
coord = MultiIndex(size=3, initial_values=[1, 2, 3])
print(f"3D coordinate: {coord}")
print(f"As list: {coord.to_list()}")

# Create a 2D coordinate for matrix access
matrix_coord = MultiIndex(size=2, initial_values=[0, 1])
...
# Create coordinate
coord = MultiIndex(size=3, initial_values=[10, 20, 30])
...
coord1 = MultiIndex(size=3, initial_values=[1, 2, 3])
coord2 = MultiIndex(size=3, initial_values=[1, 2, 3])
coord3 = MultiIndex(size=3, initial_values=[4, 5, 6])

# Equality comparison
print(f"coord1: {coord1}")
print(f"coord2: {coord2}")
print(f"coord3: {coord3}")
...
print(f"Are they equal? {coord1 == coord_copy}")

# Modify copy to show independence
coord_copy[0] = 99
print(f"\nAfter modifying copy:")
print(f"Original: {coord1}")
print(f"Copy: {coord_copy}")
```
...
## Working with Different Tensor Ranks

```{pyodide}
#| echo: true
#| output: true

# Function to demonstrate MultiIndex for different tensor types
def show_tensor_access(shape, coord_values):
    coord = MultiIndex(size=len(shape), initial_values=coord_values)

    # Calculate linear offset (row-major order)
    offset = 0
    stride = 1
    ...
    print(f"Coordinate: {coord.to_list()}")
    print(f"Linear offset: {offset}")
    print()

# 1D Vector
show_tensor_access([10], [5])

# 2D Matrix
show_tensor_access([4, 5], [2, 3])

# 3D Tensor
show_tensor_access([3, 4, 5], [1, 2, 3])

# 4D Tensor (common in deep learning: batch, channel, height, width)
show_tensor_access([2, 3, 32, 32], [1, 2, 15, 20])
```
...
def iterate_2d_tensor(rows, cols):
    ...
    for i in range(rows):
        for j in range(cols):
            coord = MultiIndex(size=2, initial_values=[i, j])
...
original = MultiIndex(size=3, initial_values=[1, 2, 3])
