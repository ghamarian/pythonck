...
graph TB
    ...
    subgraph "Transform Types"
        ...
        UNM["UnmergeTransform<br/>Multi-D → Linear"]
        MRG["MergeTransform<br/>Linear → Multi-D"]
...
# Forward transform: logical → physical
upper_coord = MultiIndex(size=2, values=[1, 2])
lower_coord = transform.calculate_lower_index(upper_coord)
print(f"\nForward: {upper_coord.to_list()} → offset {lower_coord.to_list()[0]}")
print(f"Calculation: 1×12 + 2×1 = {lower_coord.to_list()[0]}")

# Backward transform: physical → logical
lower_idx = MultiIndex(size=1, values=[14])
upper_recovered = transform.calculate_upper_index(lower_idx)
print(f"\nBackward: offset {lower_idx.to_list()[0]} → {upper_recovered.to_list()}")
...
## 2. UnmergeTransform - Multi-D to Linear
...
print(f"\n3D coord {coord_3d.to_list()} → Linear index {linear_idx.to_list()[0]}")

# Verify the calculation
idx = coord_3d.to_list()[0] * (4*2) + coord_3d.to_list()[1] * 2 + coord_3d.to_list()[2]
print(f"Verification: {coord_3d.to_list()[0]}×8 + {coord_3d.to_list()[1]}×2 + {coord_3d.to_list()[2]} = {idx}")

# Backward: 1D → 3D
linear_input = MultiIndex(size=1, values=[14])
coord_3d_result = transform.calculate_upper_index(linear_input)
print(f"\nLinear index {linear_input.to_list()[0]} → 3D coord {coord_3d_result.to_list()}")
```
...
## 3. MergeTransform - Linear to Multi-D
...
graph LR
    ...
    subgraph "Calculation"
        C["13 = 2×5 + 3<br/>row=13÷5=2, col=13%5=3"]
    end
...
print(f"Linear index {linear.to_list()[0]} → 2D coord {coord_2d.to_list()}")
print(f"Calculation: 13 = 2×5 + 3, so coord is [2, 3]")

# Backward: 2D → 1D  
coord_2d_input = MultiIndex(size=2, values=[2, 3])
linear_result = transform.calculate_upper_index(coord_2d_input)
print(f"\n2D coord {coord_2d_input.to_list()} → Linear index {linear_result.to_list()[0]}")
```
