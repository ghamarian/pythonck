...
This document illustrates key concepts of implementing a block GEMM (General Matrix Multiplication) kernel on AMD's MI300 GPU. 
...
This kernel has a very low compute throughput according `rocprofv3` profiler output. It is stalling 
on global memory read transactions effecetively starving the rest of the pipeline that needs that data to
proceed.
...
This means that even with large tiling, we can approach only ~6% of the **compute efficiency** of the theoretical maximum.
...
The below for loop will be unrolled by the compiler at compile time. It is useful
to save register that our kernel will be allocating at the runtime.

```{cpp}
template <int Begin, int End, typename Func>
__device__ __forceinline__ void static_for(Func&& f) {
...
__device__ __forceinline__ void LDS_WRITE(T* lds_storage, T* src)
{
  int tIdx = threadIdx.x;
  ...
  for (int r = 0; r < row_per_thread; r++)
  {
    ...
    T* d = lds_storage + (row + r) * lds_row_stride + col_xor*loads_per_row;
    for (int i=0; i < loads_per_row; i++)
    {
 	   d[i] = s[i];
    }
...
template <typename T, const int r_stride, int tile_size_k,
    int MRepeats, int KRepeats >
__device__ inline void LDS_READ_A(T* dst_reg,  const T* lds)
{
    int tIdx = threadIdx.x;
    ...
    static_for<0, KRepeats>([&] (auto k_c) {
      static_for<0, MRepeats>([&] (auto m_c) {
            constexpr int m = m_c.value;
            constexpr int k = k_c.value;

            const T * data = ldsFetch<T, r_stride, tile_size_k>(lds, 2*load_Ax + 8*k,
                load_Ay + ((m + MRepeats*wave0101)*16));
            // this is 128-bit wide LDS read
            *dst_reg++ = *data;
            *dst_reg++ = *(data+1);
      });
    });
...
template <typename T, const int r_stride, int tile_size_k,
    int NRepeats, int KRepeats >
__device__ inline void LDS_READ_B(T* dst_reg,  const T* lds)
{
    int tIdx = threadIdx.x;
...
float4vec  run_mfma16x16x16bf16(const float4fp16vec& a, const float4fp16vec& b, float4vec &accum)
{
...
template <typename T_IN, typename T_OUT, const int KRepeats,  const int NRepeats, const int MRepeats, const int offset>
__device__  void run_mfma16x16x16fp16(const T_IN* __restrict__ A, const  T_IN* __restrict__ B, T_OUT* __restrict__ out_f)
{
       static_for<0, MRepeats>([&](auto m) {
         static_for<0, KRepeats>([&](auto k) {
           static_for<0, NRepeats>([&](auto n) {
                constexpr int mi = m.value;
                constexpr int ni = n.value;
                constexpr int ki = k.value;

                out_f[ni + NRepeats * mi + offset] =
                ...
                out_f[ni + NRepeats * mi + offset] =
                    run_mfma16x16x16bf16(
                        A[2 * (mi + MRepeats * ki) + 1],
                        B[2 * (ni + NRepeats * ki) + 1],
                        out_f[ni + NRepeats * mi + offset]);
            });
        });
    });
...
`rocprofv3` output above shows that we have achieved our kernel to become compute bound 
where matrix multiplication operations have become the dominant workload. 