...
Let us consider a 32 bank LDS with 4 bytes per bank. Each thead will be writing 8 2-byte elements ( 16 bytes total), 
consuming 4 banks in LDS. fp16 or bf16 are the common formats  GPU kernels have to deal with.
With the phase access pattern like above by default it is a bank conflict free LDS write access.
...
This read pattern generates 4-way bank conflict in every phase of access.
...
You may find it hard to make LDS read access a fully bank-conflict free by applying a padding strategy.
...
for r in range(num_rows):
    ...
    ax.text(-1.5, -0.5, "x'", ha="center", va="center", fontsize=12, fontweight="bold")
    ax.text(num_cols + 1, -0.25, f"row r={r}", ha="left", va="center", fontsize=12, fontweight="bold")
...
fig.suptitle("XOR preshuffle mapping per row  (x' = r XOR x)", fontsize=16, y=0.92)
plt.show()
```
Below is an example that visualizes how XOR preshuffling works for LDS reads. Without preshuffling it is a 4-way
LDS read conflict, applying XOR preshuffle makes read access conflict free.
