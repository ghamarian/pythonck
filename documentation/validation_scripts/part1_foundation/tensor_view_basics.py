...
def demonstrate_tensor_view_properties():
    ...
    show_result("Tensor lengths", tensor_view.get_tensor_descriptor().get_lengths())
    show_result("Element space size", tensor_view.get_tensor_descriptor().get_element_space_size())
    show_result("Is static", tensor_view.get_tensor_descriptor().is_static())
...
def demonstrate_packed_tensor_view():
    ...
    show_result("Packed tensor lengths", tensor_view.get_tensor_descriptor().get_lengths())
...
def demonstrate_tensor_view_transforms():
    ...
    show_result("Original shape", original_view.get_tensor_descriptor().get_lengths())
    
    # Create transpose transformation
    from pytensor.tensor_adaptor import make_transpose_adaptor
    transpose_adaptor = make_transpose_adaptor(2, [1, 0])  # Transpose 2D tensor: axes 0 and 1
    show_adaptor_info(transpose_adaptor, "Transpose adaptor")
    
    # Apply transformation using adaptor components
    transposed_view = transform_tensor_view(
        original_view,
        transpose_adaptor.transforms,
        transpose_adaptor.lower_dimension_hidden_idss,
        transpose_adaptor.upper_dimension_hidden_idss
    )
    show_result("Transposed shape", transposed_view.get_tensor_descriptor().get_lengths())
...
def demonstrate_multi_dimensional_views():
    ...
    show_result("3D tensor shape", tensor_view_3d.get_tensor_descriptor().get_lengths())
    
    # Create 4D tensor view
    tensor_view_4d = make_naive_tensor_view_packed(data, [2, 2, 3, 2])
    show_result("4D tensor shape", tensor_view_4d.get_tensor_descriptor().get_lengths())
