...
**[Tensor Transform Visualizer](https://tensortransform.streamlit.app)** - Explore tensor descriptor transformations with visual graphs and mathematical formulas. See how data layouts change through various transformations.
...
<svg width="600" height="400" viewBox="0 0 600 400" xmlns="http://www.w3.org/2000/svg">
  ...
  <g>
    ...
    <text x="355" y="245" font-size="11" fill="#666">(1,2) → 1×4+2 = 6</text>
...
# Forward: Lower (2D) → Upper (1D) 
lower_2d = MultiIndex(size=2, values=[2, 3])
upper_linear = transform.calculate_upper_index(lower_2d)
print(f"2D coord {lower_2d.to_list()} → Linear index {upper_linear.to_list()[0]}")
print(f"Calculation: {lower_2d.to_list()[0]}×5 + {lower_2d.to_list()[1]} = {upper_linear.to_list()[0]}")

# Inverse: Upper (1D) → Lower (2D)
upper_input = MultiIndex(size=1, values=[13])
lower_result = transform.calculate_lower_index(upper_input)
print(f"\nLinear index {upper_input.to_list()[0]} → 2D coord {lower_result.to_list()}")
print(f"Calculation: 13 ÷ 5 = {lower_result.to_list()[0]} remainder {lower_result.to_list()[1]}")
```
...
# Forward: Lower (1D) → Upper (3D)
lower_linear = MultiIndex(size=1, values=[14])
upper_3d = transform.calculate_upper_index(lower_linear)
print(f"Linear index {lower_linear.to_list()[0]} → 3D coord {upper_3d.to_list()}")
print(f"Calculation: 14 = {upper_3d.to_list()[0]}×8 + {upper_3d.to_list()[1]}×2 + {upper_3d.to_list()[2]}")

# Inverse: Upper (3D) → Lower (1D)
upper_input = MultiIndex(size=3, values=[1, 3, 0])
lower_result = transform.calculate_lower_index(upper_input)
print(f"\n3D coord {upper_input.to_list()} → Linear index {lower_result.to_list()[0]}")
print(f"Calculation: {upper_input.to_list()[0]}×8 + {upper_input.to_list()[1]}×2 + {upper_input.to_list()[2]} = {lower_result.to_list()[0]}")
```
...
# Forward: Lower (1D) → Upper (2D)
lower_linear = MultiIndex(size=1, values=[14])
upper_coord = transform.calculate_upper_index(lower_linear)
print(f"Linear index {lower_linear.to_list()[0]} → 2D coord {upper_coord.to_list()}")
print(f"Calculation: 14 ÷ 12 = {upper_coord.to_list()[0]}, remainder = {upper_coord.to_list()[1]}")

# Inverse: Upper (2D) → Lower (1D)
upper_input = MultiIndex(size=2, values=[1, 2])
lower_result = transform.calculate_lower_index(upper_input)
print(f"\n2D coord {upper_input.to_list()} → Linear index {lower_result.to_list()[0]}")
print(f"Calculation: {upper_input.to_list()[0]}×12 + {upper_input.to_list()[1]}×1 = {lower_result.to_list()[0]}")
```
...
# Forward: Lower (0D) → Upper (2D) - all map to zeros
lower_empty = MultiIndex(size=0, values=[])
upper_coord = transform.calculate_upper_index(lower_empty)
print(f"\nForward: Empty lower → Upper {upper_coord.to_list()}")

# Inverse: Upper (2D) → Lower (0D) - all map to empty
test_coordinates = [(0, 0), (1, 2), (2, 3)]
for coord in test_coordinates:
    upper_input = MultiIndex(size=2, values=list(coord))
    lower_result = transform.calculate_lower_index(upper_input)
    print(f"Upper {upper_input.to_list()} → Lower {lower_result.to_list()} (empty)")
...
# Inverse: Upper (original) → Lower (translated) using transform
upper_coord = MultiIndex(size=1, values=[5])
lower_coord = transform.calculate_lower_index(upper_coord)
print(f"Original index {upper_coord.to_list()[0]} → Translated index {lower_coord.to_list()[0]}")
print(f"Calculation: {upper_coord.to_list()[0]} + {16} = {lower_coord.to_list()[0]}")

# Forward: Lower (translated) → Upper (original) using transform  
lower_input = MultiIndex(size=1, values=[21])
upper_result = transform.calculate_upper_index(lower_input)
print(f"\nTranslated index {lower_input.to_list()[0]} → Original index {upper_result.to_list()[0]}")
...
for idx in test_original_indices:
    ...
    print(f"  Original {idx} → Translated {translated.to_list()[0]} (offset +{16})")
...
lower_coord = transform.calculate_lower_index(upper_coord)
print(f"\nForward: {upper_coord.to_list()} → {lower_coord.to_list()} (unchanged)")

# Reverse: Lower → Upper (identity)
lower_input = MultiIndex(size=1, values=[42])
upper_result = transform.calculate_upper_index(lower_input)
print(f"Reverse: {lower_input.to_list()} → {upper_result.to_list()} (unchanged)")
```
...
for idx in test_coords:
    ...
    print(f"Upper {idx} → Lower {adjusted_idx} ({'valid' if is_valid else 'padding'})")
```
