...
struct TileDistributionExample
{
                                   ...
                                   ck_tile::index_t global_shape_1) const
    {
        ...
        // Print buffer contents (only from one thread to avoid flooding)
        if(threadIdx.x == 0 && blockIdx.x == 0) {
            printf("\nGlobal data (from global memory) shape=(%d,%d):\n\n", static_cast<int>(global_shape_0), static_cast<int>(global_shape_1));
            for(index_t i = 0; i < global_shape_0; i++) {
                for(index_t j = 0; j < global_shape_1; j++) {
                    printf("%.0f\t", global_data[i * global_shape_1 + j]);
                }
                printf("\n");
            }
            printf("\n");
...
Let's see a different tile distribution example with repeating tiles:
